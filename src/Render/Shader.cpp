...
unsigned int Shader::compileShader(unsigned int type,
  ...
  if (result == GL_FALSE) {
    ...
    char* message = (char*)alloca(
        length * sizeof(char));  // to allocate on the stack (impossible to do:
                                 // char message[length];)
...
void Shader::setUniforms4f(const std::string& name, float v0, float v1,
                           float v2, float v3) {
  // set the uniform values
  GLCall(glUniform4f(getUniformLocation(name), v0, v1, v2, v3));
}

void Shader::setUniformsMat4f(const std::string& name,
