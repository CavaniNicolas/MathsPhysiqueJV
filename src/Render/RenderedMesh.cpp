...
RenderedMesh::RenderedMesh(Mesh mesh, std::string textureFilepath):
    m_va(VertexArray()),
    m_vb(VertexBuffer(mesh.getVertices())),
    m_ib(IndexBuffer(mesh.getIndices(), sizeof(*mesh.getIndices()) / sizeof(unsigned int))),
    m_layout(VertexBufferLayout()),
    m_texture(Texture(textureFilepath)),
...
void RenderedMesh::transalte(glm::vec3 translation)
{
    // translate the model
