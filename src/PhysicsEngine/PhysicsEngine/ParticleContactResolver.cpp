...
void ParticleContactResolver::resolveContacts(std::vector<std::shared_ptr<ParticleContact>>& contactArray)
{
    ...
    std::priority_queue<std::shared_ptr<ParticleContact>,
                        std::vector<std::shared_ptr<ParticleContact>>,
                        ParticleContact::HighestSeparatingVelocity>
      queue(contactArray.begin(), contactArray.end());
