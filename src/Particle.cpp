...
Particle::Particle(const Particle& other): 
	Particle(other.m_position, other.m_velocity, other.m_acceleration, other.m_inverseMass)
{}
...
void Particle::integrate() {
	if (m_lastIntegrationTime != 0) {
		...
		double deltaT = (double)(currentTime - m_lastIntegrationTime) / CLOCKS_PER_SEC;
		m_position.setCoord(m_position.getX() + deltaT * m_velocity.getX() + 0.5 * pow(deltaT, 2) * m_acceleration.getX(),
							m_position.getY() + deltaT * m_velocity.getY() + 0.5 * pow(deltaT, 2) * m_acceleration.getY(),
							m_position.getZ() + deltaT * m_velocity.getZ() + 0.5 * pow(deltaT, 2) * m_acceleration.getZ());
